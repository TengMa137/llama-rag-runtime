...
rag::Metadata metadata_from_arrays(const char* const* keys, const char* const* values,
                                   std::size_t count) {
    ...
    if (count > max_tags)
        throw std::runtime_error("metadata/filter tag limit exceeded");
    if (count != 0 && (!keys || !values))
        throw std::runtime_error("metadata/filter arrays are required");
    rag::Metadata result;
    std::size_t total = 0;
    for (std::size_t i = 0; i < count; ++i) {
        if (!keys[i] || !values[i])
            throw std::runtime_error("metadata/filter entries must be non-null");
        const std::string key(keys[i]);
        const std::string value(values[i]);
        if (key.empty() || key.size() > max_key_bytes || value.size() > max_value_bytes ||
            total > max_total_bytes - key.size() ||
            total + key.size() > max_total_bytes - value.size())
            throw std::runtime_error("invalid metadata/filter field size");
        total += key.size() + value.size();
        if (!result.emplace(key, value).second)
            throw std::runtime_error("duplicate metadata/filter key");
    }
    return result;
}
...
extern "C" int
lrs_index_stage_upsert_with_metadata(const lrs_index* current, const lrs_index_options* options,
    ...
    if (!current || !options || !document_id || !title || !content || !candidate || !unchanged ||
        (metadata_count != 0 && (!metadata_keys || !metadata_values)))
        return fail(error, "invalid upsert arguments");
    try {
        auto metadata = metadata_from_arrays(metadata_keys, metadata_values, metadata_count);
        ...
        auto added =
            next->corpus().upsert_document(document_id, normalized, std::move(metadata), title);
        if (!added)
...
extern "C" int lrs_index_stage_upsert(const lrs_index* current, const lrs_index_options* options,
                                      ...
                                      char** error) {
    return lrs_index_stage_upsert_with_metadata(current, options, document_id, title, content,
                                                nullptr, nullptr, 0, candidate, unchanged, error);
}
...
extern "C" int lrs_index_search_filtered_json(const lrs_index* index, const char* query,
                                              const char* mode, size_t top_k,
    ...
    if (!index || !query || !mode || !json || top_k == 0 || top_k > 100 ||
        (filter_count != 0 && (!filter_keys || !filter_values)))
        return fail(error, "invalid search arguments");
    try {
        auto filter =
            exact_match_filter(metadata_from_arrays(filter_keys, filter_values, filter_count));
        std::vector<rag::SearchResult> results;
        const std::string selected(mode);
        if (selected == "lexical") {
            const auto hits = filter ? index->engine->corpus().lexical_search(query, top_k, filter)
                                     : index->engine->corpus().lexical_search(query, top_k);
            for (const auto& h : hits)
                results.push_back(index->engine->corpus().resolve(h));
        } else if (selected == "dense") {
            auto hits = filter ? index->engine->corpus().dense_search(query, top_k, filter)
                               : index->engine->corpus().dense_search(query, top_k);
            if (!hits)
            ...
            auto hits = pipeline.run(index->engine->corpus(), query, top_k, filter);
            if (!hits)
                return fail(error, hits.error().message);
            for (const auto& h : *hits)
                results.push_back(index->engine->corpus().resolve(h));
...
extern "C" int lrs_index_search_json(const lrs_index* index, const char* query, const char* mode,
    ...
    return lrs_index_search_filtered_json(index, query, mode, top_k, nullptr, nullptr, 0, json,
                                          error);
}

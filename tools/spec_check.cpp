...
std::string revision(const std::string& root, const std::string& dependency) {
  const std::string command = "git -C \"" + root + "/third_party/" + dependency + "\" rev-parse HEAD";
  FILE* pipe = popen(command.c_str(), "r");
  if (!pipe) throw std::runtime_error("cannot inspect dependency revision");
  char buffer[128]{};
  const std::string value = std::fgets(buffer, sizeof(buffer), pipe) ? buffer : "";
  const int status = pclose(pipe);
  if (status != 0) throw std::runtime_error("cannot inspect " + dependency + " revision");
...
int main() {
  try {
    ...
    if (revision(root, "llama.cpp") != LRS_LLAMA_PIN || revision(root, "rag-cpp") != LRS_RAGCPP_PIN) {
    ...
    while (std::getline(spec, line)) {
      ++number;
      if (line.rfind("## 24.", 0) == 0) acceptance = true;
      if (line.rfind("## 25.", 0) == 0) acceptance = false;
      if (line == "Exit criteria:") exit_criteria = true;
      else if (exit_criteria && (line.rfind("### ", 0) == 0 || line.rfind("## ", 0) == 0)) exit_criteria = false;

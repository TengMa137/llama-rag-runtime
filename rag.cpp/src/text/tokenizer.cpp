...
bool is_vowel(const std::string& s, std::size_t i) {
    switch (s[i]) {
        ...
        case 'u':
            return true;
        case 'y':
            return i == 0 ? false : !is_vowel(s, i - 1);
        default:
            return false;
    }
}
...
bool ends_with(const std::string& s, std::string_view suf) {
    return s.size() >= suf.size() && std::string_view(s).substr(s.size() - suf.size()) == suf;
}
...
std::string porter_stem(std::string_view word) {
    ...
    if (ends_with(s, "eed")) {
        ...
        if (!stem.empty() && measure(stem, stem.size() - 1) > 0)
            s.resize(s.size() - 1);
    ...
    if (ends_with(s, "y")) {
        ...
        if (has_vowel(stem, stem.empty() ? 0 : stem.size() - 1))
            s[s.size() - 1] = 'i';
    }
    ...
    for (auto [suf, rep] : step2)
        if (replace(s, suf, rep, m_gt0))
            break;
    ...
    if (ends_with(s, "e")) {
        ...
        if (!stem.empty()) {
            ...
            if (m > 1 || (m == 1 && !cvc(stem, stem.size() - 1)))
                s = stem;
        }
    }
    // Step 5b

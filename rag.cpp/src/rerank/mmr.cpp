...
std::vector<Hit> mmr(const index::Corpus& corpus, std::span<const Hit> candidates, MmrConfig cfg) {
    ...
    for (std::size_t i = 0; i < n; ++i)
        rel[i] = (candidates[i].score.get() - lo) / span;
    ...
    for (std::size_t step = 0; step < k; ++step) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            for (std::size_t s : order)
                max_sim = std::max(max_sim, sim(i, s));
...
class MmrStage final : public pipeline::RetrievalStage {
  public:
    MmrStage(float lambda, std::string label) : lambda_(lambda), label_(std::move(label)) {}
...
pipeline::StagePtr make_mmr_stage(float lambda, std::string label) {

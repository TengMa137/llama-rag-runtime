...
std::size_t Bm25Index::add(std::uint32_t id, std::string_view text) {
    ...
    for (auto& [term, freq] : tf) {
        ...
        Posting p{id, freq};
        auto it =
            std::lower_bound(plist.begin(), plist.end(), p,
                             [](const Posting& a, const Posting& b) { return a.doc < b.doc; });
        if (it != plist.end() && it->doc == id)
            it->tf = freq; // overwrite on re-add
        else
            plist.insert(it, p);
    }
        ...
        for (const auto& [term, plist] : postings_) {
            ...
            for (const auto& p : plist) {
                ...
                if (!dense_len_.empty() && p.doc < dense_len_.size()) {
                    if (const std::uint32_t raw = dense_len_[p.doc])
                        dl = static_cast<float>(raw);
...
float Bm25Index::idf(std::size_t n_t) const {
    ...
    return static_cast<float>(
        std::log(1.0 + (N - static_cast<double>(n_t) + 0.5) / (static_cast<double>(n_t) + 0.5)));
}
...
void Bm25Index::term_coverage(const std::vector<std::string>& q_terms,
    ...
    for (const auto& t : uniq) {
        ...
        while (pi < plist.size() && si < slot.size()) {
            if (plist[pi].doc < slot[si].first)
                ++pi;
            else if (slot[si].first < plist[pi].doc)
                ++si;
            else {
                ++out[slot[si].second];
                ++pi;
...
float Bm25Index::score_doc(const std::vector<std::string>& q_terms, std::uint32_t doc_id) const {
    auto dl_it = doc_len_.find(doc_id);
    if (dl_it == doc_len_.end())
        return 0.0f;
...
std::vector<Hit> Bm25Index::search(std::string_view query, std::size_t k) const {
    ...
    if (!dense_len_.empty() && !pw_.empty()) {
        ...
        for (std::uint32_t d : touched)
            hits.push_back(Hit{ChunkId{d}, Score{acc[d]}});
    ...
    for (const auto& term : q_terms) {
        ...
        for (const auto& p : plist) {
            ...
            const float dl = dl_it == doc_len_.end() ? avgdl : static_cast<float>(dl_it->second);
            const float f = static_cast<float>(p.tf);
            const float num = f * (k1 + 1.0f);
            const float den = f + k1 * (1.0f - b + b * dl * inv_avgdl);
            acc[p.doc] += term_idf * (num / den);
    ...
    for (auto& [doc, sc] : acc)
        hits.push_back(Hit{ChunkId{doc}, Score{sc}});
...
Result<Bm25Index> Bm25Index::deserialize(std::string_view blob) {
    try {
        ...
        if (!get(blob, magic) || magic != kMagic)
            return fail<Bm25Index>(Errc::corrupt_index, "bad magic");
        if (!get(blob, version) || version != kVersion)
            return fail<Bm25Index>(Errc::corrupt_index, "bad version");
        if (!get(blob, idx.params_.k1) || !get(blob, idx.params_.b) ||
            !std::isfinite(idx.params_.k1) || !std::isfinite(idx.params_.b) ||
            idx.params_.k1 < 0.0f || idx.params_.b < 0.0f || idx.params_.b > 1.0f)
            return fail<Bm25Index>(Errc::corrupt_index, "params");
        std::uint64_t total;
        if (!get(blob, total))
            return fail<Bm25Index>(Errc::corrupt_index, "total_len");
        idx.total_len_ = static_cast<double>(total);

        std::uint32_t ndocs;
        if (!get(blob, ndocs) || ndocs > store::kMaxChunks || ndocs > blob.size() / 8)
            return fail<Bm25Index>(Errc::corrupt_index, "ndocs");
        idx.doc_len_.reserve(ndocs);
        for (std::uint32_t i = 0; i < ndocs; ++i) {
            std::uint32_t doc, len;
            if (!get(blob, doc) || !get(blob, len))
                return fail<Bm25Index>(Errc::corrupt_index, "doc_len");
            if (!idx.doc_len_.emplace(doc, len).second)
                return fail<Bm25Index>(Errc::corrupt_index, "duplicate doc length");
        }
        std::uint32_t nterms;
        if (!get(blob, nterms) || nterms > store::kMaxPostings || nterms > blob.size() / 8)
            return fail<Bm25Index>(Errc::corrupt_index, "nterms");
        idx.postings_.reserve(nterms);
        for (std::uint32_t i = 0; i < nterms; ++i) {
            std::string term;
            std::uint32_t np;
            if (!get_str(blob, term) || term.empty() || !get(blob, np) ||
                np > store::kMaxPostings || np > blob.size() / sizeof(Posting))
                return fail<Bm25Index>(Errc::corrupt_index, "term");
            std::vector<Posting> plist(np);
            std::uint32_t previous = 0;
            for (std::size_t posting_index = 0; posting_index < plist.size(); ++posting_index) {
                auto& p = plist[posting_index];
                if (!get(blob, p.doc) || !get(blob, p.tf) || p.tf == 0 ||
                    (posting_index != 0 && p.doc <= previous))
                    return fail<Bm25Index>(Errc::corrupt_index, "posting");
                previous = p.doc;
            }
            if (!idx.postings_.emplace(std::move(term), std::move(plist)).second)
                return fail<Bm25Index>(Errc::corrupt_index, "duplicate term");
        }
        if (!blob.empty())
            return fail<Bm25Index>(Errc::corrupt_index, "trailing bm25 data");
        idx.finalize();
        return idx;
    } catch (const std::exception& error) {
        return fail<Bm25Index>(Errc::corrupt_index,
                               std::string("bm25 parse failed: ") + error.what());
    } catch (...) {
        return fail<Bm25Index>(Errc::corrupt_index, "bm25 parse failed");
    }
}

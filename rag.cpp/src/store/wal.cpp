...
bool decode(std::string_view payload, WalRecord& out) {
    ...
    if (op != static_cast<std::uint8_t>(WalOp::add_document) &&
        op != static_cast<std::uint8_t>(WalOp::remove_document))
        return false;
    out.op = static_cast<WalOp>(op);
    if (!r.str(out.uri) || !r.str(out.title) || !r.str(out.text))
        return false;
    std::uint32_t n = 0;
    if (!r.u(n) || n > kMaxMetadataEntries || n > r.remaining() / 8)
        return false;
    out.meta.clear();
    ...
    return r.u(out.doc_id) && r.remaining() == 0;
}
...
Wal::Wal(Wal&& o) noexcept
    : fd_(std::exchange(o.fd_, -1)), path_(std::move(o.path_)), mode_(o.mode_),
...
Result<void> Wal::open(std::string path, SyncMode mode) {
    ...
    if (fd < 0)
        return fail<void>(Errc::io_error, "wal open " + path + ": " + std::strerror(errno));
    fd_ = fd;
    path_ = std::move(path);
    mode_ = mode;
...
Result<void> Wal::sync_now() noexcept {
    switch (mode_) {
        ...
        case SyncMode::flush:
            if (::fsync(fd_) != 0)
                return fail<void>(Errc::io_error, "wal fsync");
            return {};
        case SyncMode::full:
...
Result<void> Wal::append(const WalRecord& rec) {
    try {
        if (fd_ < 0)
            return fail<void>(Errc::io_error, "wal not open");

        const std::string payload = encode(rec);
        if (payload.size() > kMaxWalRecordBytes || payload.size() > UINT32_MAX)
            return fail<void>(Errc::invalid_argument, "wal record exceeds configured limit");
        Writer w;
        ...
        while (off < frame.size()) {
            ...
            if (n < 0) {
                ...
                return fail<void>(Errc::io_error,
                                  std::string("wal write: ") + std::strerror(errno));
            }
            off += static_cast<std::size_t>(n);
        }
        if (auto s = sync_now(); !s)
            return s;
        bytes_ += frame.size();
        return {};
    } catch (const std::exception& error) {
        return fail<void>(Errc::io_error, std::string("wal append failed: ") + error.what());
    } catch (...) {
        return fail<void>(Errc::io_error, "wal append failed");
    }
}
...
Result<std::vector<WalRecord>> Wal::replay(const std::string& path) {
    try {
        ...
        if (!in)
            return out; // absent log = nothing to replay
        std::stringstream ss;
        ss << in.rdbuf();
        ...
        return fail<std::vector<WalRecord>>(Errc::corrupt_index,
                                            std::string("wal replay failed: ") + error.what());
    } catch (...) {
        return fail<std::vector<WalRecord>>(Errc::corrupt_index, "wal replay failed");
    }
}
